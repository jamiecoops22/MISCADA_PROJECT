...
double distN(double *coords, int n, int p, int indx_1, int indx_2);
...
void mkNNIndx(int n, int P, int m, double *coords, int *nnIndx, double *nnDist, int *nnIndxLU);
...
double spCor(double &D, double &phi, double &nu, int &covModel, double *bk){
  ...
  if(covModel == 0){//exponential
    ...
    cout << "c++ error: cov.model is not correctly specified" << endl;
  }
}


//Description: update B and F.
double updateBF(double *B, double *F, double *c, double *C, double *D, double *d, int *nnIndxLU, int *CIndx, int n, double *theta, int tauSqIndx, int sigmaSqIndx, int phiIndx, int nuIndx, int covModel, double *bk, double nu_b){
  ...
  int nb = 1+static_cast<int>(floor(nu_b));
  int threadID = 0;
...
#pragma omp parallel for private(k, l, info, threadID)
	...
	for(k = 0; k < nnIndxLU[n+i]; k++){
	  c[nnIndxLU[i]+k] = theta[sigmaSqIndx]*spCor(d[nnIndxLU[i]+k], theta[phiIndx], theta[nuIndx], covModel, &bk[threadID*nb]);//exp(-theta[phiIndx]*d[nnIndxLU[i]+k]);
	  for(l = 0; l <= k; l++){
	    C[CIndx[i]+l*nnIndxLU[n+i]+k] = theta[sigmaSqIndx]*spCor(D[CIndx[i]+l*nnIndxLU[n+i]+k], theta[phiIndx], theta[nuIndx], covModel, &bk[threadID*nb]);//exp(-theta[phiIndx]*D[CIndx[i]+l*nnIndxLU[n+i]+k]);
	    if(l == k){
...
int main(int argc, char **argv){
  ...
  int covModel; par.getVal("cov.model", covModel);
  ...
  double nu_a; par.getVal("nu.a", nu_a);
  double nu_b; par.getVal("nu.b", nu_b);
  double nuStarting; par.getVal("nu.starting", nuStarting);
  double nuTuning; par.getVal("nu.tuning", nuTuning);
  ...
  double *coords = NULL; par.getFile("coords.file", coords, n, P);
  ...
  if(!nnFast){
    ...
    mkNNIndx(n, P, m, coords, nnIndx, d, nnIndxLU);
...
#pragma omp parallel for private(iNNIndx, iNN)
  ...
  for(i = 0; i < n; i++){
	...
	D[CIndx[i]+l*nnIndxLU[n+i]+k] = distN(coords, n, P, nnIndx[nnIndxLU[i]+k], nnIndx[nnIndxLU[i]+l]);
      }
    }
  }
  ...
  double *bk = new double[nThreads*(static_cast<int>(1.0+nu_b))];
  
  bool thetaUpdate = true;

  //update B and F
  logDetCurrent = updateBF(B, F, c, C, D, d, nnIndxLU, CIndx, n, theta, tauSqIndx, sigmaSqIndx, phiIndx, nuIndx, covModel, bk, nu_b);
  ...
  for(s = 0; s < nSamples; s++){
    ...
    if(covModel == 2){
      logPostCurrent += log(theta[nuIndx] - nu_a) + log(nu_b - theta[nuIndx]);
    }
    ...
    if(covModel == 2){
      thetaCand[nuIndx] = logitInv(rnorm(logit(theta[nuIndx], nu_a, nu_b), nuTuning), nu_a, nu_b);
    }
    ...
    //update B and F
    logDetCand = updateBF(B, F, c, C, D, d, nnIndxLU, CIndx, n, thetaCand, tauSqIndx, sigmaSqIndx, phiIndx, nuIndx, covModel, bk, nu_b);
    ...
    if(covModel == 2){
      logPostCand += log(thetaCand[nuIndx] - nu_a) + log(nu_b - thetaCand[nuIndx]);
    }
...
double distN(double *coords, int n, int p, int indx_1, int indx_2){
  ...
  for(i = 0; i < p; i++){
    dist += pow(coords[i*n+indx_1] - coords[i*n+indx_2], 2);
  }
